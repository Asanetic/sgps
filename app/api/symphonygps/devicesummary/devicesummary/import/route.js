...
import { AddDevicesummary } from "../DevicesummaryDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("device_list", node, req, newRecordId, authData);

     const insertResult = await AddDevicesummary(newRecordId, mutatedObj, req, authData);
