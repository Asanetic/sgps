...
export async function POST(DevicegpslogsRequest) {
  ...
  //--- End gps_logs inputs array --//
    ...
    if (DevicegpslogsFormAction === "add_gps_logs") 
    {
    ...
    if (DevicegpslogsFormAction === "update_gps_logs") {
      
      // update table Devicegpslogs
      const result = await UpdateDevicegpslogs(newId, mutatedDataArray, body, authData, `primkey='${gps_logs_uptoken_value}'`)
    ...
    return Response.json({
      status: 'error',
      message: `Invalid action: ${DevicegpslogsFormAction}`
    }, { status: 400 });
