...
import { AddAssetalarms } from "../AssetalarmsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("asset_alarms", node, req, newRecordId, authData);

     const insertResult = await AddAssetalarms(newRecordId, mutatedObj, req, authData);
