...
import { AddRegisteredsites } from "../RegisteredsitesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("sites", node, req, newRecordId, authData);

     const insertResult = await AddRegisteredsites(newRecordId, mutatedObj, req, authData);
